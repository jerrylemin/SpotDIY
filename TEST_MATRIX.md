...
| Frontend behavior | `npm test` | Pass: 102 Vitest tests across 28 files, including Spotify source-matching readiness and existing playback/library/lyrics behavior. |
...
| Rust quality | `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings` | Existing unrelated warnings remain in `src/sources/local.rs` and `src/lyrics/providers.rs`; Spotify/download changes introduce no reported warning. |
| Rust behavior | `cargo test --manifest-path src-tauri/Cargo.toml --all-targets` | Pass locally: 446 unit tests plus the real-MPV integration test, including schema 12 preservation, Spotify source matching, playback, persistence, Windows, backup, analytics, visual, preview, and regression coverage. |
| Browser playback | `pnpm exec playwright test` | Pass: 82 tests across 1280, 1920, 2560, and Plan 15 ultrawide projects. |
...
| Provider contracts | Native/frontend tests and direct spotdl smoke | Pass: local search, provider isolation, Spotify `spotdl` metadata/source matching, missing-tool states, and cancellation boundary; live downloads skipped without an approved legal fixture. |
| Persistence boundary | Schema/restart package matrix | Pass: schema 12 and format 1 are current; schema 9-to-12 migrations and restart boundaries pass, with private/temporary state non-durable. |
| Visual follow-up | 82 Playwright tests and Plan 15 package smoke | Pass: visual routes, keyboard/focus, capability actions, local preview, Theme Studio, and responsive guards. |
...
## Current Spotify source-matching overlay — 2026-09-05

The working tree repairs YouTube canonical URL fallback and download capability,
SoundCloud alternate URL validation, bounded search and audio-only modes,
truthful native download readiness, structured download errors, `spotdl`
Spotify metadata/source matching, the schema 9-to-12 migrations,
case-insensitive WebM scanning, validated online MPV playback, and persisted
MPV/yt-dlp/FFmpeg configuration with actionable missing-tool behavior. Focused
native tests, frontend tests, the full browser matrix, the external-target
Tauri/NSIS build, real-MPV smoke, packaged search smoke, and Plan 15 packaged
visual smoke pass. Direct `spotdl --version` and real Spotify URL-resolution
smoke pass; live audio download acceptance remains skipped without an approved
legal fixture.
