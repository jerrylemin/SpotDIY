-- Replace the original download provider check so Spotify search results can
-- be source-matched by spotdl and downloaded through the existing yt-dlp worker.
CREATE TABLE downloads_spotify_v12 (
    id TEXT PRIMARY KEY NOT NULL CHECK (length(trim(id)) > 0),
    provider_kind TEXT NOT NULL
        CHECK (provider_kind IN ('youtube', 'soundcloud', 'spotify')),
...
INSERT INTO downloads_spotify_v12 (
...
ALTER TABLE downloads_spotify_v12 RENAME TO downloads;
...
-- The PKCE opt-in is no longer part of the settings contract.
DELETE FROM settings_metadata
WHERE setting_key = 'spotify_catalog_enabled';

UPDATE schema_metadata
SET metadata_value = '12', updated_at = '1970-01-01T00:00:00Z'
