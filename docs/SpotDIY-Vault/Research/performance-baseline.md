...
## 2026-09-04 runtime repair status

The Plan 16 runtime-usability repair did not change these measurements or
claim performance closure. Provider readiness, MPV configuration, WebM
recognition, and persisted settings behavior are verified independently; the
release status remains `PARTIAL` until the broad process-tree and timed native
render gates are addressed.
