...
Date: 2026-09-04. This matrix covers the 27 requested feature groups. A
...
|---|---|---|---|---|---|
| 1 | App bootstrap / shell | CI package, packaged smoke, E2E | `scripts/packaged-playback-smoke.ps1 -Plan11Shell`; Playwright 82/82 | PASS | Clean startup and close verified. |
| 2 | Database / migration | CI native, packaged migration | Rust all-target tests; schema-9-to-11 migration tests; packaged Plan 11/12/14 migration smokes | PASS | Schema 11; migrations preserve existing settings rows and foreign keys, default Spotify opt-in off, and add persisted media-tool paths. |
...
| 4 | Search providers | CI native/frontend, packaged | `src-tauri/src/search`; provider tests; `scripts/provider-search-smoke.ps1 -RunPackaged` | PASS | YouTube/SoundCloud canonical URL and capability contracts, local/provider isolation, missing yt-dlp states, Spotify gate, and cancellation pass; live upstream checks skipped. |
...
| 6 | Main playback | CI native, 60-second package, E2E | `src-tauri/src/playback`; regular package smoke; real-mpv smoke; 60-second harness reached `60059 ms` | PASS | Playback continuity, seek, pause/resume, shutdown, and owned-mpv cleanup pass. Deterministic missing-MPV failure does not enter recovery cycles; performance budget is separate and fails. |
...
| 11 | Download manager | CI native/frontend | `src-tauri/src/downloads.rs`; download tests; Rust all-target tests | PASS | Native readiness, YouTube/SoundCloud mode policy, structured errors, queue, bounded progress, cancel/retry/recovery, provenance, and path safety pass; live download skipped. |
...
the release acceptance fixture was not enabled; Spotify `SKIPPED` because authorization is not
available; live download `SKIPPED` because no approved legal fixture exists.

Plan 16 runtime-usability repair evidence is current as of 2026-09-04: local
Rust all-target tests, frontend quality gates, 82 Playwright tests, external
Tauri/NSIS packaging, explicit real-MPV smoke, packaged search smoke, and
packaged Plan 15 visual/restart smoke pass. This does not close the separate
