...
Standard mode targets `%LOCALAPPDATA%\SpotDIY`; portable mode keeps its data beside the executable. Plan 12 delivers the capability- and OS-tested Mini overlay, global shortcut, tray, media control, output profile, always-on-top, and native-drag boundary.
...
`WindowsIntegrationService` owns one lazy Tauri overlay window: Mini. Its
native label is `overlay-mini`; Rust owns its exact size, close/reopen reuse,
always-on-top configuration, and native dragging. The overlay capability is
intentionally limited to event listen/unlisten, window close, always-on-top,
and native-drag operations required by the surface.

The tray menu and seven typed global shortcuts dispatch to the same native
...
per binding. The Mini overlay remains a standard always-on-top window and can
be moved by dragging its non-control area.
...
window visibility, handles, tray state, SMTC runtime objects, and native drag
state are not persisted.

The packaged Plan 12 smoke proves fresh schema-8 startup, native tray/status,
live SMTC, controlled shortcut registration, Mini overlay reuse/topmost,
seek/volume controls, output-profile apply/restore, restart persistence, and
zero owned mpv processes. Browser preview remains native-free and truthful.
