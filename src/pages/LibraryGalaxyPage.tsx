...
export function LibraryGalaxyPage() {
  ...
  const request = { ...EMPTY_REQUEST, query: query.trim() || null, genre: genre || null, artist: artist || null, likedOnly: false };
  ...
  useEffect(() => {
    ...
    for (const point of layout.points) if (!clusterAnchors.has(point.clusterKey)) clusterAnchors.set(point.clusterKey, point);
    for (const cluster of layout.clusters) {
       const point = clusterAnchors.get(cluster.key);
      if (!point) continue;
      const x = point.x * view.scale + view.x;
      const y = point.y * view.scale + view.y;
      context.fillStyle = muted;
       context.fillText(cluster.label, x + 14, y - 14);
    }
  ...
  return (
    <div className="page-stack visual-page galaxy-page">
      <section className="page-intro"><div><span className="eyebrow">EXPLORE / LIBRARY GALAXY</span><h1>Your collection, <em>in orbit.</em></h1><p>Each dot is one local track. Clusters group those tracks by the mode you choose, so the shape of your library is easy to scan.</p></div><div className="page-intro-stat"><strong>{layout.points.length}</strong><span>tracks plotted</span></div></section>
